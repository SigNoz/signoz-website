...
const K8sHotrod = () => {
  return (
    <Paragraph>
      ...
      , and generate tracing data. 1. Use the HotROD install script below to
      create a `sample-application` namespace and deploy HotROD application on
      it:
      <Code>
        curl -sL
        https://github.com/SigNoz/signoz/raw/main/sample-apps/hotrod/hotrod-install.sh
        \ | HELM_RELEASE=my-release SIGNOZ_NAMESPACE=platform bash
      </Code>
      2. Using the `kubectl -n sample-application get pods` command, monitor the
      sample application pods. Wait for all the pods to be in running state:
      <Code>kubectl -n sample-application get pods</Code>
      Output:
      <Code>
        NAME READY STATUS RESTARTS AGE hotrod-55bd58cc8d-mzxq8 1/1 Running 0 2m
      ...
      <Code>
        ...
        \ --restart=&apos;OnFailure&apos; -i --tty --rm --command -- curl -X
        POST -F \ &apos;locust_count=6&apos; -F &apos;hatch_rate=2&apos;
        http://locust-master:8089/swarm
      </Code>
      4. Browse to `http://localhost:3301` and see the metrics and traces for
      your sample application. 5. Use the following command to stop load
      generation:
      <Code>

...
![](/images/docs/add-panel-time-series.png)


Enter the title of your new panel. The example screenshot creates a new panel named “CPU Utilization”:

![](/images/docs/new-panel-cpu-utilization.png)

Choose `RATE_AVG` as the aggregate function:

![](/images/docs/choose-rate-avg.png)

```
  Info:
  For details on aggregate functions, see the Aggregate Function section.
```

From the **Metrics** drop-down, select `system_cpu_time`:

![](/images/docs/select-system-cpu-time.png)

Specify that the state should not be `idle` by adding a filtering condition as shown below:

![](/images/docs/state-nin-idle.png)

In the **Legend Format** text box, enter `{{state}}`:

![](/images/docs/legend-format-enter-state.png)

Use the **Y Axis Unit** dropdown to specify that the value displayed on the Y-axis is a percentage and can take values from 0 to 100:

![](/images/docs/y-axis-percentage.png)

To preview your query, select the **Stage & Run Query** button. SigNoz will plot a graph similar to the following one:

![](/images/docs/preview-cpu-utilization.png)

_(Optional)_ To further drill down, you can plot the a separate graph for each state by specifying `state` in the **Group By** drop-down and selecting the **Stage & Run Query** button. SigNoz will plot a graph similar to the following one:
...
Enter the title of your new panel. The example screenshot creates a new panel named “Disk Saturation”:

![](/images/docs/new-panel-disk-saturation.png)

Choose `RATE` as the aggregate function:
...
From the **Metrics** drop-down, select `system_weighted_io_time`:

![](/images/docs/select-system_weighted_io_time.png)

In the **Legend Format** text box, enter `{{device}}`:
...
(Optional) To further drill down, you can plot the a separate graph for each host name by specifying host_name in the **Group By** drop-down and selecting the **Stage & Run Query** button.
...
Enter the title of your new panel. The example screenshot creates a new panel named “Network Errors”:

![](/images/docs/new-panel-network-errors.png)

From the **Metrics** drop-down, select `system_network_errors`:

![](/images/docs/select-system_network_errors.png)

_(Optional)_ Specify that you want to see the network errors for a specific host (this example uses `signoz-host`) by adding a filtering condition as shown below:
...
In the **Legend Format** text box, enter `{{device}}-{{direction}}`:
...
_(Optional)_ To further drill down, you can plot the a separate graph for sending and receiving data by specifying `direction` in the **Group By** drop-down and selecting the **Stage & Run Query** button.
...
Enter the title of your new panel. The example screenshot creates a new panel named “Requests per Service”:

![](/images/docs/new-panel-requests-per-service.png)

Choose `SUM_RATE` as the aggregate function:
...
Use the **Metrics** drop-down to specify that you want to plot the total number requests made to your application. The example screenshot below plots the total number of requests made to SigNoz:

![](/images/docs/signoz-total-calls.png)

Specify the the services that you want to plot.
...
_(Optional)_ If you are plotting all the services, enter `{{service_name}}` in the **Legend Format** text box.
...
Enter the title of your new panel. The example screenshot creates a new panel named “Latency per Service”:

![](/images/docs/new-panel-latency-per-service.png)

**Specify the First Query (A)**
...
8. When you’ve finished, select the **Save** button at the top right corner of the screen. Then, select the **OK** button to confirm.
